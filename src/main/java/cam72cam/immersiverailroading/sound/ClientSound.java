...
public class ClientSound implements ISound {
	...
	public void play(Vec3d pos) {
		...
		if (repeats || currentPos == null || Minecraft.getMinecraft().thePlayer == null) {
			sndSystem.get().play(id);
		} else if (Minecraft.getMinecraft().thePlayer.getPositionVector().distanceTo(currentPos) < this.attenuationDistance * 1.1) {
			sndSystem.get().play(id);
		}
	}
	...
	public void update() {
		...
		if (currentPos != null) {
			snd.CommandQueue(new CommandObject(CommandObject.SET_POSITION, id, (float)currentPos.xCoord, (float)currentPos.yCoord, (float)currentPos.zCoord));
		}
