	...
	public static ITrack findTrack(World world, Vec3d currentPosition, float trainYaw, double gauge) {
		...
		for (Vec3d pos : positions) {
			for (double height : heightSkew) {
				ITrack te = Util.getTileEntity(world, pos.addVector(0, height + (currentPosition.yCoord%1), 0), true);
				if (te != null && Gauge.from(te.getTrackGauge()) == Gauge.from(gauge)) {
					return te;
				}
				// HACK for cross gauge
				TileRailBase rail = TileRailBase.get(world, new BlockPos(pos.addVector(0, height + (currentPosition.yCoord%1), 0)));
