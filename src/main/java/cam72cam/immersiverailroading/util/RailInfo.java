		...
		if (builder.canBuild()) {
			if (!world.isRemote) {
				...
				for (ItemStack playerStack : player.inventory.mainInventory) {
					...
					if (settings.railBed != null && settings.railBed.getItem() == playerStack.getItem() && settings.railBed.getMetadata() == playerStack.getMetadata()) {
						bed += playerStack.stackSize;
					}
					if (settings.railBedFill != null && settings.railBedFill.getItem() == playerStack.getItem() && settings.railBedFill.getMetadata() == playerStack.getMetadata()) {
				...
				if (settings.railBed != null && bed < builder.costBed()) {
					player.addChatMessage(ChatText.BUILD_MISSING_RAIL_BED.getMessage(builder.costBed() - bed));
					isOk = false;
				}
				
				if (settings.railBedFill != null && fill < builder.costFill()) {
