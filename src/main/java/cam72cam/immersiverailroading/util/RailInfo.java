	...
	private class MaterialManager {
		...
		private boolean checkMaterials(EntityPlayer player) {
			...
			if (found < count) {
				...
				player.addChatMessage(ChatText.BUILD_MISSING.getMessage(count - found, example));
				return false;
			}
			return true;
		}
