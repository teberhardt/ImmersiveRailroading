...
public abstract class BlockRailBase extends Block {
	...
	public void onNeighborChange(IBlockAccess world, BlockPos pos, BlockPos neighbor){
		...
		while (true) {
            ...
            tileEntity.setWorldObj(syncd.getWorld());
		...
		if (te != null) {
			if (stack.getItem() == IRItems.ITEM_SWITCH_KEY) {
				...
				if (tileSwitch != null) {
					...
					if (!worldIn.isRemote) {
						playerIn.addChatMessage(switchForced.equals(SwitchState.NONE) ? new TextComponentString(ChatText.SWITCH_UNLOCKED.toString()) : ChatText.SWITCH_LOCKED.getMessage(switchForced.toString()));
					}
				}
			}
