...
public class StockModel extends OBJRender {
		...
		if (def.getComponent(RenderComponentType.BOGEY_POS, "FRONT", stock.gauge) != null) {
			...
			if (wheels != null) {
				for (RenderComponent wheel : wheels) {
					...
					GlStateManager.pushMatrix();
					GlStateManager.translate(wheelPos.xCoord, wheelPos.yCoord, wheelPos.zCoord);
	...
	private void drawSteamLocomotive(LocomotiveSteam stock) {
		...
		case TRI_WALSCHAERTS:{
			...
			drawWalschaerts(stock, "CENTER", -120, wheel.height(), wheels.get(0).center(), wheels.get(0).center(), false);
			break;
		}
        ...
        case GARRAT:
			{
                ...
                Vec3d frontPos = stock.predictFrontBogeyPosition((float) (-frontVec.xCoord - def.getBogeyFront(stock.gauge)));
                Vec3d frontNext = stock.predictFrontBogeyPosition((float) (-frontVec.xCoord - def.getBogeyFront(stock.gauge) - 0.5));
                ...
                if (frontPos.distanceTo(frontNext) > 0.1) {
					GL11.glTranslated(frontVec.xCoord, frontVec.yCoord, frontVec.zCoord);
	...
	private void drawBogies(EntityMoveableRollingStock stock) {
			...
			GL11.glPushMatrix();
			{
				...
				if (frontBogeyWheels != null) {
					for (RenderComponent wheel : frontBogeyWheels) {
						...
						GL11.glPushMatrix();
						{
							GL11.glTranslated(wheelPos.xCoord, wheelPos.yCoord, wheelPos.zCoord);
		...
		if (rearBogey != null)
		{
			...
			Vec3d rearPos = stock.predictRearBogeyPosition((float) (-rearVec.xCoord - def.getBogeyRear(stock.gauge)));
			Vec3d rearNext = stock.predictRearBogeyPosition((float) (-rearVec.xCoord - def.getBogeyRear(stock.gauge) - 0.5));
			...
			GL11.glPushMatrix();
			{
				...
				if (rearPos.distanceTo(rearNext) > 0.1) {
					GL11.glTranslated(rearVec.xCoord, rearVec.yCoord, rearVec.zCoord);
	...
	private void drawStephenson(LocomotiveSteam stock, String side, int wheelAngleOffset, double diameter, Vec3d wheelCenter, Vec3d wheelPos) {
		...
		double connRodRadius = connRodPos.xCoord - wheelCenter.xCoord;
		...
		GL11.glPushMatrix();
		{
			...
			GL11.glTranslated(connRodMovment.xCoord, connRodMovment.zCoord, 0);
		...
		Vec3d drivingRodRotPoint = new Vec3d(drivingRod.max().xCoord - drivingRod.height()/2, drivingRod.center().yCoord, drivingRod.max().zCoord);
		// Angle for movement height vs driving rod length (adjusted for assumed diameter == height, both sides == 2r)
		float drivingRodAngle = (float) Math.toDegrees(MathHelper.atan2(connRodMovment.zCoord, drivingRod.length() - drivingRod.height()));
		...
		GL11.glPushMatrix();
		{
			...
			GL11.glTranslated(drivingRodRotPoint.xCoord, drivingRodRotPoint.yCoord, drivingRodRotPoint.zCoord);
			// Rotate rod angle
		...
		double pistonDelta = connRodMovment.xCoord - connRodRadius;
	...
	private void drawWalschaerts(LocomotiveSteam stock, String side, int wheelAngleOffset, double diameter, Vec3d wheelCenter, Vec3d wheelPos, boolean reverse) {
		...
		Vec3d drivingRodRotPoint = new Vec3d((reverse ? drivingRod.min() : drivingRod.max()).xCoord - drivingRod.height()/2, drivingRod.center().yCoord, (reverse ? drivingRod.min() : drivingRod.max()).zCoord);
		// Angle for movement height vs driving rod length (adjusted for assumed diameter == height, both sides == 2r)
		float drivingRodAngle = (float) Math.toDegrees(MathHelper.atan2((reverse ? -connRodMovment.zCoord : connRodMovment.zCoord), drivingRod.length() - drivingRod.height()));
		...
		Vec3d returnCrankRotPoint = reverse ?
				returnCrank.min().addVector(returnCrank.height()/2, returnCrank.height()/2, 0) :
				returnCrank.max().addVector(-returnCrank.height()/2, -returnCrank.height()/2, 0);
		Vec3d wheelRotationOffset = reverse ?
				VecUtil.fromWrongYaw(returnCrankRotPoint.xCoord - wheelPos.xCoord, (float) wheelAngle) :
				VecUtil.fromWrongYaw(returnCrankRotPoint.xCoord - wheelPos.xCoord, (float) wheelAngle);
		Vec3d returnCrankOriginOffset = wheelPos.addVector(wheelRotationOffset.xCoord, wheelRotationOffset.zCoord, 0);
		double returnCrankAngle = wheelAngle + 90 + 30;
		GL11.glPushMatrix();
		{
			// Move to crank offset from origin
			GL11.glTranslated(returnCrankOriginOffset.xCoord, returnCrankOriginOffset.yCoord, 0);
			// Rotate crank
			GL11.glRotated(returnCrankAngle, 0, 0, 1);
			// Draw return crank at current position
			GL11.glTranslated(-returnCrankRotPoint.xCoord, -returnCrankRotPoint.yCoord, 0);
		...
		Vec3d returnCrankRodOriginOffset = returnCrankOriginOffset.addVector(returnCrankRotationOffset.xCoord, returnCrankRotationOffset.zCoord, 0);
		// Point about which the return crank rotates
		Vec3d returnCrankRodRotPoint = reverse ?
				returnCrankRod.min().addVector(returnCrankRod.height()/2, returnCrankRod.height()/2, 0) :
				returnCrankRod.max().addVector(-returnCrankRod.height()/2, -returnCrankRod.height()/2, 0);
		...
		double slottedLinkLowest = slottedLink.min().yCoord + slottedLink.width()/2;
		// Fudge
		double returnCrankRodFudge = reverse ?
				Math.abs(slottedLink.center().xCoord - (returnCrankRodOriginOffset.xCoord + returnCrankRodLength))/3 :
                Math.abs(slottedLink.center().xCoord - (returnCrankRodOriginOffset.xCoord - returnCrankRodLength))/3;
		float returnCrankRodRot = reverse ?
				-VecUtil.toWrongYaw(new Vec3d(slottedLinkLowest - returnCrankRodOriginOffset.yCoord + returnCrankRodFudge, 0, returnCrankRodLength)) :
				VecUtil.toWrongYaw(new Vec3d(slottedLinkLowest - returnCrankRodOriginOffset.yCoord + returnCrankRodFudge, 0, returnCrankRodLength));
		// Angle the return crank rod should be at to hit the slotted link
		GL11.glPushMatrix();
		{
			// Move to crank rod offset from origin
			GL11.glTranslated(returnCrankRodOriginOffset.xCoord, returnCrankRodOriginOffset.yCoord, 0);
			
			GL11.glRotated(returnCrankRodRot, 0, 0, 1);
			
			// Draw return crank rod at current position
			GL11.glTranslated(-returnCrankRodRotPoint.xCoord, -returnCrankRodRotPoint.yCoord, 0);
		...
		Vec3d returnCrankRodFarPoint = returnCrankRodOriginOffset.addVector(returnCrankRodRotationOffset.xCoord, returnCrankRodRotationOffset.zCoord, 0);

		// Slotted link rotation point
		Vec3d slottedLinkRotPoint = slottedLink.center();
		double slottedLinkRot = Math.toDegrees(MathHelper.atan2(-slottedLinkRotPoint.xCoord + returnCrankRodFarPoint.xCoord, slottedLinkRotPoint.yCoord - returnCrankRodFarPoint.yCoord));
		GL11.glPushMatrix();
		{
			// Move to origin
			GL11.glTranslated(slottedLinkRotPoint.xCoord, slottedLinkRotPoint.yCoord, 0);
		...
		double forwardMax = (slottedLink.min().yCoord - slottedLinkRotPoint.yCoord) * 0.4;
		double forwardMin = (slottedLink.max().yCoord - slottedLinkRotPoint.yCoord) * 0.65;
		...
		throttleSlotPos += slottedLinkRotPoint.yCoord - radiusBar.max().yCoord;
		...
		GL11.glPushMatrix();
		{
			...
			GL11.glTranslated(radiusBarClose.xCoord, radiusBarClose.yCoord, 0);
		...
		float combinationLeverAngle = VecUtil.toWrongYaw(new Vec3d(delta.xCoord, 0, delta.yCoord));

		GL11.glPushMatrix();
		{
			GL11.glTranslated(pistonDelta, 0, 0);
			GL11.glTranslated(combinationLeverRotPos.xCoord, combinationLeverRotPos.yCoord, 0);
