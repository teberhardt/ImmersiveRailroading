...
public class EntityBuildableRollingStock extends EntityRollingStock {
		...
		for (int i = 0; i < player.inventory.getSizeInventory(); i ++) {
			...
			if (found != null && found.getItem() == IRItems.ITEM_PLATE) {
				if (ItemGauge.get(found) == this.gauge) {
					...
					case LARGE:
						largePlates+=found.stackSize;
						break;
					case MEDIUM:
						mediumPlates+=found.stackSize;
						break;
					case SMALL:
						smallPlates+=found.stackSize;
						break;
					default:
						break;
					}
	...
	public ItemComponentType removeNextComponent(EntityPlayer player) {
		...
		worldObj.spawnEntityInWorld(new EntityItem(worldObj, player.posX, player.posY, player.posZ, item));
