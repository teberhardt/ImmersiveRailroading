...
public abstract class Locomotive extends FreightTank {
	...
	public void handleKeyPress(Entity source, KeyTypes key, boolean sprinting) {
		...
		case DEAD_MANS_SWITCH:
			if (deadManChangeTimeout == 0) { 
				...
				if (deadMansSwitch) {
					source.addChatMessage(ChatText.DEADMANS_SWITCH_ENABLED.getMessage());
				} else {
					source.addChatMessage(ChatText.DEADMANS_SWITCH_DISABLED.getMessage());
				}
	...
	public boolean processInitialInteract(EntityPlayer player, ItemStack stack, EnumHand hand) {
		if (player.getHeldItem(hand) != null && player.getHeldItem(hand).getItem() == IRItems.ITEM_RADIO_CONTROL_CARD) {
			if(this.gauge.isModel() || this.getDefinition().getRadioCapability() || !Config.ConfigBalance.RadioEquipmentRequired) {
				...
				if (player.isSneaking()) {
					if (!cardNBT.hasKey("linked_uuid")) {
						player.addChatMessage(ChatText.RADIO_NOLINK.getMessage());
					} else {
						cardNBT.removeTag("linked_uuid");
						player.addChatMessage(ChatText.RADIO_UNLINK.getMessage());
					}
				} else {
					if (!cardNBT.hasKey("linked_uuid")) {
						cardNBT.setString("linked_uuid",this.getPersistentID().toString());
						player.addChatMessage(ChatText.RADIO_LINK.getMessage());
					} else {
						cardNBT.setString("linked_uuid",this.getPersistentID().toString());
						player.addChatMessage(ChatText.RADIO_RELINK.getMessage());
					}
				}
			}
			else {
				player.addChatMessage(ChatText.RADIO_CANT_LINK.getMessage(this.getName()));;
			}
			return true;
		}
	...
	public float ambientTemperature() {
		float mctemp = worldObj.getBiome(this.getPosition()).getFloatTemperature(getPosition());
