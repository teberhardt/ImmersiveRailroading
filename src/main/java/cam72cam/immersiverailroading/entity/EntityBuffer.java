...
public abstract class EntityBuffer extends EntityRidableRollingStock {
	...
	public EntityBuffer(World world, String defID) {
		super(world, defID);
	}
	...
	public void onUpdate() {
	    ...
	    if (this.getCurrentSpeed().metric() > 1) {
			...
			for (Entity entity : entitiesWithin) {
				if (entity instanceof EntityBuffer) {
					// rolling stock collisions handled by looking at the front and
					// rear coupler offsets
					continue;
				} 
	
				if (entity.getRidingEntity() instanceof EntityBuffer) {
