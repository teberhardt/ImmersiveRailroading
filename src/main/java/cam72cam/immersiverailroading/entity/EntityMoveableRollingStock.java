...
public abstract class EntityMoveableRollingStock extends EntityRidableRollingStock {
	...
	public void onUpdate() {
	    ...
	    this.posX = skewScalar(this.posX, currentPos.position.xCoord);
	    this.posY = skewScalar(this.posY, currentPos.position.yCoord);
	    this.posZ = skewScalar(this.posZ, currentPos.position.zCoord);
					...
					if (!BlockUtil.isIRRail(worldObj, bp)) {
						...
						if (bb.intersectsWith(bbb)) { // This is slow, do it as little as possible
							if (!BlockUtil.isIRRail(worldObj, bp.up())) {
								worldObj.destroyBlock(bp, Config.ConfigDamage.dropSnowBalls || !(state.getBlock() == Blocks.SNOW || state.getBlock() == Blocks.SNOW_LAYER));
							}
						}
					} else {
						TileRailBase te = TileRailBase.get(worldObj, bp);
						if (te != null) {
							te.cleanSnow();
							continue;
						}
	...
	public TickPos moveRollingStock(double moveDistance, int lastTickID) {
		...
		return new MovementSimulator(worldObj, lastPos, this.getDefinition().getBogeyFront(gauge), this.getDefinition().getBogeyRear(gauge), gauge.value()).nextPosition(moveDistance);
	}
	...
	public class PosRot extends Vec3d {
		...
		public PosRot(Vec3d nextFront, float yaw) {
			this(nextFront.xCoord, nextFront.yCoord, nextFront.zCoord, yaw);
		}
	...
	public Vec3d predictFrontBogeyPosition(TickPos pos, float offset) {
		MovementSimulator sim = new MovementSimulator(worldObj, pos, this.getDefinition().getBogeyFront(gauge), this.getDefinition().getBogeyRear(gauge), gauge.value());
	...
	public PosRot predictRearBogeyPosition(TickPos pos, float offset) {
		MovementSimulator sim = new MovementSimulator(worldObj, pos, this.getDefinition().getBogeyRear(gauge), this.getDefinition().getBogeyRear(gauge), gauge.value());
	...
	public int getSpeedRetarderSlowdown(TickPos latest) {
		...
		for (Vec3d pos : this.getDefinition().getBlocksInBounds(gauge)) {
			...
			try {
				TileEntity potentialTE = worldObj.getChunkFromBlockCoords(bp).getTileEntity(bp, EnumCreateEntityType.CHECK);
				if (potentialTE != null && potentialTE instanceof TileRailBase) {
					...
					if (te.getAugment() == Augment.SPEED_RETARDER) {
						max = Math.max(max, RedstoneUtil.getPower(worldObj, bp));
						over += 1;
					}
				}
			} catch (Exception ex) {
