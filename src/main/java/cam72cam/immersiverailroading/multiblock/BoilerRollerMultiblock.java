	...
	public class BoilerRollerInstance extends MultiblockInstance {
		...
		public boolean onBlockActivated(EntityPlayer player, EnumHand hand, BlockPos offset) {
			if (!player.isSneaking()) {
				...
				} else if (held != null && held.getItem() == IRItems.ITEM_PLATE && ItemPlateType.get(held) == PlateType.BOILER) {
					TileMultiblock craftTe = getTile(crafting);
					if (craftTe == null) {
						return false;
					}
		...
		public void tick(BlockPos offset) {
			...
			if (progress == 0) {
				...
				if (input != null && input.getItem() == IRItems.ITEM_PLATE && ItemPlateType.get(input) == PlateType.BOILER && output == null) {
					progress = 100;
