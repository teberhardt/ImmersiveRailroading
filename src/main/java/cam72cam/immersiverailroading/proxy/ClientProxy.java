	...
	public static void onClick(MouseEvent event) {
		...
		if ((event.getButton() == attackID || event.getButton() == useID) && event.isButtonstate()) {
			...
			if (entity != null && entity instanceof EntityRidableRollingStock) {
				ImmersiveRailroading.net.sendToServer(new MousePressPacket(button, entity.worldObj.provider.getDimension(), entity.getEntityId()));
			...
			if (riding != null && riding instanceof EntityRidableRollingStock) {
				ImmersiveRailroading.net.sendToServer(new MousePressPacket(button, riding.worldObj.provider.getDimension(), riding.getEntityId()));
				event.setCanceled(true);
				return;
			}
		}
	...
	public static void onRenderMouseover(DrawBlockHighlightEvent event) {
		...
		if (event.getTarget().typeOfHit == RayTraceResult.Type.BLOCK) {
			...
			if (event.getPlayer().getHeldItemMainhand().getItem() == IRItems.ITEM_TRACK_BLUEPRINT) {
		        ...
		        float hitX = (float)(vec.xCoord - pos.getX());
		        float hitY = (float)(vec.yCoord - pos.getY());
		        float hitZ = (float)(vec.zCoord - pos.getZ());
		        ...
		        RailInfo info = new RailInfo(player.worldObj, stack, new PlacementInfo(stack, player.getRotationYawHead(), pos, hitX, hitY, hitZ), null);
		        ...
		        GL11.glPushMatrix();
				{
	                ...
	                GL11.glTranslated(placementPosition.xCoord-d0, placementPosition.yCoord-d1, placementPosition.zCoord-d2);
