...
public class TileRail extends TileRailBase {
	...
	public void nextTablePos(boolean back) {
		...
		int length = info.settings.length;
		List<EntityCoupleableRollingStock> ents = worldObj.getEntitiesWithinAABB(EntityCoupleableRollingStock.class, new AxisAlignedBB(-length, 0, -length, length, 5, length).offset(this.getPos()));
	...
	public void readFromNBT(NBTTagCompound nbt) {
		...
		if (nbt.hasKey("info")) {
			info = new RailInfo(worldObj, pos, nbt.getCompoundTag("info"));
		} else {
			// LEGACY
			// TODO REMOVE 2.0
			...
			ItemStack railBed = ItemStack.loadItemStackFromNBT(nbt.getCompoundTag("railBed"));
			...
			RailSettings settings = new RailSettings(gauge, "default", type, length, quarters, TrackPositionType.FIXED, TrackDirection.NONE, railBed, null, false, false);
			info = new RailInfo(worldObj, settings, placementInfo, null, switchState, switchForced, tablePos);
		}
	}
