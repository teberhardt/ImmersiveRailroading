...
public class TileRailPreview extends SyncdTileEntity implements ITickable {
	...
	public void setCustomInfo(PlacementInfo info) {
		...
		if (customInfo != null) {
			...
			double lx = Math.abs(customInfo.placementPosition.xCoord - placementInfo.placementPosition.xCoord);
			double lz = Math.abs(customInfo.placementPosition.zCoord - placementInfo.placementPosition.zCoord);
			double length;
	...
	@Override
	public void update() {
		if (!worldObj.isRemote && isMulti()) {
			ChunkManager.flagEntityPos(worldObj, pos);
