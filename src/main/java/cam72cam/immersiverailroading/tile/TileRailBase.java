...
public class TileRailBase extends SyncdTileEntity implements ITrack, ITickable {
	...
	public double getRenderGauge() {
		...
		if (this.getParentReplaced() != null && worldObj != null) {
			parent = TileRail.get(worldObj, this.getParentReplaced());
            if (parent != null) {
	...
	public Vec3d getNextPosition(Vec3d currentPosition, Vec3d motion) {
		...
		while(tile != null) {
			...
			Vec3d potential = MovementTrack.nextPosition(worldObj, currentPosition, tile, rotationYaw, distanceMeters);
	...
	public void transferAllItems(IItemHandler source, IItemHandler dest, int numstacks) {
		for (int slot = 0; slot < source.getSlots(); slot++) {
			...
			int orig_count = stack.stackSize;
			stack = ItemHandlerHelper.insertItem(dest, stack, false);
			if (stack.stackSize != orig_count) {
				source.extractItem(slot, orig_count - stack.stackSize, false);
				numstacks --;
			}
			if (numstacks <= 0) {
				return;
	...
	private <T> List<T> getCapsNearby(Capability<T> cap) {
		...
		for (EnumFacing facing : EnumFacing.values()) {
			...
			if (worldObj.isAirBlock(npos) || BlockUtil.isIRRail(worldObj, npos)) {
				continue;
			}
			TileEntity nte = worldObj.getTileEntity(npos);
			if (nte == null) {
				continue;
			}
	...
	public void update() {
		...
		if (ticksExisted > 1 && (ticksExisted % (20 * 5) == 0 || blockUpdate)) {
			...
			if (this.getParent() == null || !worldObj.isBlockLoaded(this.getParent())) {
				return;
			}
	...
	public SwitchState cycleSwitchForced() {
		...
		if (tileSwitch != null) {
			...
			tileSwitch.info = new RailInfo(worldObj, tileSwitch.info.settings, tileSwitch.info.placementInfo, tileSwitch.info.customInfo, tileSwitch.info.switchState, newForcedState, tileSwitch.info.tablePos);
