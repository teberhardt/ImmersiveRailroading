    ...
    public List<PosStep> getPath(double stepSize) {
		...
		for(int i = 0; i < points.size(); i++) {
			...
			if (points.size() == 1) {
				...
				pitch = (float) Math.toDegrees(Math.atan2(next.yCoord - p.yCoord, next.distanceTo(p)));
                yaw = curve.angleStop();
			} else if (i == 0) {
				Vec3d next = points.get(i+1);
				pitch = (float) -Math.toDegrees(Math.atan2(next.yCoord - p.yCoord, next.distanceTo(p)));
				...
				Vec3d next = points.get(i+1);
				pitch = (float) -Math.toDegrees(Math.atan2(next.yCoord - prev.yCoord, next.distanceTo(prev)));
		...
		if (subBuilders == null) {
			...
			for (BuilderBase curve : subBuilders.subList(0, Math.min(subBuilders.size(), 3))) {
				...
				for (VecYawPitch rd : curve.getRenderData()) {
					rd = new VecYawPitch(rd.xCoord + offset.xCoord, rd.yCoord + offset.yCoord, rd.zCoord + offset.zCoord, rd.yaw, rd.pitch, rd.length);
					data.add(rd);
				}
			}
			return data;
