...
public abstract class BuilderIterator extends BuilderBase implements IIterableTrack {
	...
	public BuilderIterator(RailInfo info, BlockPos pos, boolean endOfTrack) {
		...
		float heightOffset = (float) ((info.placementInfo.placementPosition.yCoord) % 1);
		...
		Vec3d placeOff = new Vec3d(
				...
				Math.abs(MathUtil.trueModulus(info.placementInfo.placementPosition.xCoord, 1)),
				0,
                Math.abs(MathUtil.trueModulus(info.placementInfo.placementPosition.zCoord, 1))
		);
		int mainX = (int) Math.floor(path.get(path.size()/2).xCoord+placeOff.xCoord);
		int mainZ = (int) Math.floor(path.get(path.size()/2).zCoord+placeOff.zCoord);
		...
		for (PosStep cur : path) {
			...
			for (double q = -horiz; q <= horiz; q+=0.1) {
				...
				int posX = (int)Math.floor(gagPos.xCoord+nextUp.xCoord+placeOff.xCoord);
				int posZ = (int)Math.floor(gagPos.zCoord+nextUp.zCoord+placeOff.zCoord);
				double height = 0;
				if (info.settings.isGradeCrossing) {
				...
				double relHeight = gagPos.yCoord % 1;
				if (gagPos.yCoord < 0) {
					relHeight += 1;
				}
				...
				if (!positions.contains(gag)) {
					...
					yOffset.put(gag, (int) (gagPos.yCoord - relHeight));
				}
			...
			if (!isFlex && endOfTrack) {
				mainX = (int) Math.floor(gagPos.xCoord+placeOff.xCoord);
				mainZ = (int) Math.floor(gagPos.zCoord+placeOff.zCoord);
			}
		}
	...
	public List<VecYawPitch> getRenderData() {
		...
		if (switchStraight ) {
			for (int i = 0; i < points.size(); i++) {
				...
				if (Math.abs(flatPos.zCoord) >= 0.5 * scale) {
					switchSize = i;
					break;
				}
			}
		...
		for (int i = 0; i < points.size(); i++) {
			...
			if (angle != 0) {
				if (direction == TrackDirection.RIGHT) {
					data.add(new VecYawPitch(switchPos.xCoord, switchPos.yCoord, switchPos.zCoord, switchPos.yaw, switchPos.pitch, (1 - angle / 180) * (float) info.settings.gauge.scale(), "RAIL_LEFT"));
					data.add(new VecYawPitch(cur.xCoord, cur.yCoord, cur.zCoord, cur.yaw, cur.pitch, (1 + angle / 180) * (float) info.settings.gauge.scale(), "RAIL_RIGHT"));
				} else {
					data.add(new VecYawPitch(cur.xCoord, cur.yCoord, cur.zCoord, cur.yaw, cur.pitch, (1 - angle / 180) * (float) info.settings.gauge.scale(), "RAIL_LEFT"));
					data.add(new VecYawPitch(switchPos.xCoord, switchPos.yCoord, switchPos.zCoord, switchPos.yaw, switchPos.pitch, (1 + angle / 180) * (float) info.settings.gauge.scale(), "RAIL_RIGHT"));
				}
				data.add(new VecYawPitch(cur.xCoord, cur.yCoord, cur.zCoord, cur.yaw, cur.pitch, "RAIL_BASE"));
			} else {
				data.add(new VecYawPitch(cur.xCoord, cur.yCoord, cur.zCoord, cur.yaw, cur.pitch));
			}
		}
