...
public class BuilderTurnTable extends BuilderBase {
	...
	public BuilderTurnTable(RailInfo info, BlockPos pos, boolean endOfTrack) {
		...
		for (double irad = 1; irad <= radius + 1; irad++) {
			for (double angle = 0; angle < 360; angle+=0.5) {
				...
				positions.add(Pair.of((int)gagPos.xCoord, (int)gagPos.zCoord));
			}
		}
	...
	public List<VecYawPitch> getRenderData() {
		...
		for (float angle = 0; angle < 360; angle +=22.5) {
			...
			data.add(new VecYawPitch(gagPos.xCoord + offset.getX(), gagPos.yCoord + offset.getY(), gagPos.zCoord + offset.getZ(), -angle));
		}
	...
	public int costTies() {
		return (int) Math.ceil((this.info.length + 8) * ConfigBalance.TieCostMultiplier);
	}
	
	public int costRails() {
		return (int) Math.ceil((this.info.length + 8)*2/3 * ConfigBalance.RailCostMultiplier);
	}
	...
	public int costFill() {
		...
		return (int) Math.ceil(this.info.railBedFill.getItem() != null ? fillCount : 0);
	}
}